...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; user-select: none; -webkit-user-select: none; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }
        ...
        #tool-bar { top: 10px; left: 10px; display: flex; flex-direction: column; gap: 6px; }
        .btn { background: #2a2a2a; color: white; border: 1px solid #444; padding: 6px 10px; border-radius: 8px; font-size: 11px; font-weight: bold; cursor: pointer; }
        .btn-admin { background: #d32f2f; border-color: #f44336; }
        .btn:active { background: #444; }

        /* Admin Paneli Gizli Kutu */
        #admin-panel { display: none; top: 50px; left: 10px; background: rgba(30, 0, 0, 0.95); border: 1px solid #f44336; padding: 10px; border-radius: 8px; width: 200px; flex-direction: column; gap: 6px; z-index: 20; }
        #admin-panel h4 { color: #ff5252; font-size: 12px; border-bottom: 1px solid #f44336; padding-bottom: 4px; }

        /* Bilgi Çubuğu (Piksel Bilgisi) */
        #pixel-info { position: absolute; top: 10px; right: 10px; background: rgba(0,0,0,0.8); padding: 5px 10px; border-radius: 6px; font-size: 10px; border: 1px solid #555; display: none; z-index: 10; }
...
<body>
    ...
    <div id="canvas-wrapper">
        ...
        <div id="tool-bar" class="ui-panel">
            <button class="btn" id="undo-btn">↩ Geri Al</button>
            <button class="btn" id="check-void-btn">🎯 Void Kontrol</button>
            <button class="btn btn-admin" id="admin-login-btn">👑 Admin Giriş</button>
        </div>

        <!-- Admin Özel Kontrol Paneli -->
        <div id="admin-panel" class="ui-panel">
            <h4>👑 YAPIMCI ADMIN PANELİ</h4>
            <button class="btn" id="admin-infinite-btn">⚡ Sınırsız Piksel: KAPALI</button>
            <button class="btn" id="admin-rb-btn">🧹 Son Alanı Sil (RB)</button>
            <button class="btn" id="admin-mute-btn">🔇 Chat Mute / Ban</button>
            <button class="btn" id="admin-clear-btn">⚠️ Tüm Haritayı Sıfırla</button>
        </div>

        <!-- Piksel Log Bilgisi -->
        <div id="pixel-info">X: 0, Y: 0 | Sahibi: Oyuncu</div>
    ...
    <script>
        ...
        let voidSecondsLeft = 7200;
        let voidActive = false;
        let voidArea = { x: 450, y: 450, size: 100 };

        // Admin Değişkenleri
        let isAdmin = false;
        let isInfinitePixels = false;
        let isChatMuted = false;
        const ADMIN_PASSWORD = "1234"; // Yapımcı Admin Şifren

        let history = [];
        let pixelLogs = {}; // Kimin nereye bastığını kaydeder
        let selectedColor = '#e74c3c';
        const colors = ['#e74c3c', '#e67e22', '#f1c40f', '#2ecc71', '#3498db', '#9b59b6', '#ffffff', '#000000'];
        ...
        adminLoginBtn.onclick = () => {
            if (!isAdmin) {
                const pass = prompt("Yapımcı Admin Şifresini Girin:");
                if (pass === ADMIN_PASSWORD) {
                    isAdmin = true;
                    adminPanel.style.display = "flex";
                    pixelInfoEl.style.display = "block";
                    alert("👑 Admin Girişi Başarılı!");
                    addChatMessage("Sistem", "👑 Oyuna Admin (Yapımcı) katıldı.");
                } else {
                    alert("Hatalı şifre!");
                }
            } else {
        ...
        document.getElementById('admin-infinite-btn').onclick = function() {
            isInfinitePixels = !isInfinitePixels;
            this.innerText = isInfinitePixels ? "⚡ Sınırsız Piksel: AÇIK" : "⚡ Sınırsız Piksel: KAPALI";
        };

        document.getElementById('admin-rb-btn').onclick = () => {
            if (history.length === 0) return;
            for(let i = 0; i < 10; i++) {
                if(history.length > 0) {
                    const last = history.pop();
                    ctx.fillStyle = last.color;
                    ctx.fillRect(last.x, last.y, last.size, last.size);
                }
            }
            alert("🧹 Son 10 piksel Rollback (RB) ile silindi.");
        };

        document.getElementById('admin-mute-btn').onclick = function() {
            isChatMuted = !isChatMuted;
            this.innerText = isChatMuted ? "🔊 Chat Mute: AÇIK" : "🔇 Chat Mute / Ban";
            addChatMessage("Sistem", isChatMuted ? "🚫 Sohbet Admin tarafından kapatıldı." : "✅ Sohbet tekrar açıldı.");
        };

        document.getElementById('admin-clear-btn').onclick = () => {
            if (confirm("Tüm haritayı sıfırlamak istediğinize emin misiniz?")) {
                ctx.fillStyle = '#ffffff';
                ctx.fillRect(0, 0, CANVAS_SIZE, CANVAS_SIZE);
                history = [];
                alert("Harita tamamen temizlendi!");
            }
        };

        // Piksel Koyma ve Log Görme
        canvas.addEventListener('click', (e) => {
            const rect = canvas.getBoundingClientRect();
            const scaleX = canvas.width / rect.width;
            const scaleY = canvas.height / rect.height;
            ...
            // Admin için Piksel Logunu Ekranın Sağ Üstünde Göster
            if (isAdmin) {
                const owner = pixelLogs[`${x},${y}`] || "Boş / Beyaz";
                pixelInfoEl.innerText = `X: ${x}, Y: ${y} | Sahibi: ${owner}`;
            }

            if (!isInfinitePixels && availablePixels <= 0) return;
            ...
            // Log Kaydı Tut
            pixelLogs[`${x},${y}`] = isAdmin ? "👑 Admin" : "Oyuncu";
        ...
        checkVoidBtn.onclick = () => {
            ...
            addAmount = 1;
            voidActive = false;
            addChatMessage("Sistem", "🎉 TEBRİKLER! Void alanı kazanıldı (+1 piksel moduna geçildi).");
        };

        // 59 Saniye Piksel Yenilenme
        setInterval(() => {
            currentTimer--;
            if (currentTimer <= 0) {
                availablePixels += addAmount;
        ...
        setInterval(() => {
            if (voidSecondsLeft > 0) {
                ...
                voidSecondsLeft = 7200;
                voidActive = true;
                ctx.fillStyle = '#111111';
                ctx.fillRect(voidArea.x, voidArea.y, voidArea.size, voidArea.size);
                addChatMessage("Sistem", "⚠️ YENİ VOID ALANI DOĞDU!");
            }
        }, 1000);
        ...
        chatSend.onclick = () => {
            if (isChatMuted && !isAdmin) {
                alert("Sohbet Admin tarafından kısıtlandı!");
                return;
            }
            const text = chatInput.value.trim();
            if (text !== "") {
                addChatMessage(isAdmin ? "👑 Admin" : "Oyuncu", text);
                chatInput.value = "";
            }
        };
